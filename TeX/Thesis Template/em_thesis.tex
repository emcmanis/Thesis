...
\eqn{
f'(x_n) + \frac{1}{6}f'''(x_n)\Delta{x}^2 + O(\Delta{x}^3) = \frac{f(x_n + \Delta x) - f(x_n - \Delta x)}{2 \Delta x}
\label{fdifference}
}
\eqn{
f''(x_n) + \frac{1}{12}f''''(x_n)\Delta{x}^2 + O(\Delta{x}^3) = {f(x_n + \Delta x) - 2 f(x_n) + f(x_n - \Delta x)}{\Delta x^2}
\label{f2difference}
}
Applied to a general 2nd-order-ODE $f''(x) + a(x)f'(x) + b(x)f(x) = c(x)$, this is\footnote{this is all out of Joel's sci-comp notes}
\eqn{
f_{n-1} \left( -\frac{a_n}{2\Delta x} + \frac{1}{\Delta x^2} \right) + f_n\left(b_n - \frac{2}{\Delta x^2} \right) + f_{n+1}\left(\frac{a_n}{2\Delta x} + \frac{1}{\Delta x^2} \right) = c_n
}
which suggests rewriting this as the multiplication of a vector with a column vector equal to some source term. Put all of the $f(x_n)$s together, and you've got a large matrix with entries on the diagonal and off-diagonal multiplying a column vector equal to some other column vector.

For our potential, we want to set up the eigenvalue problem. Our source vector on the right side will then be $E u$. To deal with the end points, we solve the equation at $x_0$ and $x_f$ and modify the source appropriately. We know that the wave function will be 0 at $x_0$ -- that's the center of mass, and $V(r)$ blows up. Because we are looking for bound states, we demand that the wave function go to 0 at infinity, which means that $x_f$ will also be 0. 

Our finite difference matrix thus runs from $\rho = 0$ to $\rho_{\infty}$, that is, whatever we declare numerical infinity to be. 

Using \eqref{eq:fdifference} and \eqref{eq:f2difference}, we discretize our Hamiltonian. $E u_n$ will be given by
\eqn{
\left(-\frac{1}{\Delta \rho^2}, \frac{2}{\Delta \rho^2} - \frac{2}{n \Delta \rho} e^{-n\Delta \rho / \lambda}, -\frac{1}{\Delta \rho^2}\right)
\left(
\begin{array}{c}
u_{n-1} \\
u_{n} \\
...
\begin{figure}[h]
\centering
\subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/n2yukawa}}
\subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/n2hyd}}
...
\caption{A bound state at $\lambda = 200$, probably $n = 14$. This plot shows the whole range where anything interesting is going on -- again, the function has gone to 0 and will stay there.}
